...
这份计划记录项目真正要解决的问题：FileSage 不只是按扩展名搬文件，而是一个可中断、可恢复、可解释的文件分析与整理系统。

## 核心定位

FileSage 的目标是帮助用户理解、比较、去重、归档和整理复杂文件集合。

它的产品身份是：证据驱动、懂用户偏好的智能文件整理高级顾问。

它确实是智能整理文件，但不是“AI 猜一个分类，然后 best effort 搬文件”。它的整理链路是：
...
- 是否只产出报告，而不执行任何整理动作。
- 用户希望通过自然语言表达偏好，还是通过高级 settings 精确配置。

系统本质上有一个底层 File Organization Runtime。AI 接口、精确 settings、CLI、agent API 和 GUI 都是 runtime 上面的控制层。

## 基本原则

### 0. 智能整理高级顾问

不要把项目写成普通 AI 文件分类器，也不要把它写成产品降级后的小工具。
...
- 理解你想保留什么质量的版本。
- 理解你不希望它碰哪些东西。
- 理解什么时候只要报告，什么时候生成方案，什么时候允许执行。
- 理解什么时候证据不足，需要继续分析或问你。

“懂你”不是 AI 自称懂你，而是通过自然语言偏好、显式 settings、review decisions、preference model 和 runtime policy 共同实现。
...
用户用自然语言表达偏好，AI 生成规则草案，用户确认后进入 runtime policy。

高级用户：

> 你可以直接在 settings 里配置精确规则。

高级用户直接定义 similarity predicates、traversal policy、risk threshold 和 workflow。

两条入口必须进入同一个 File Organization Runtime，不能形成两套不一致的判断系统。

### 7.6 不做 best effort

项目不接受“best effort organize”作为目标。

- 证据不足就输出 `unknown`。
- 需要更深分析就输出 `needs_more_analysis`。
- 风险高就输出 `require_confirmation`。
- 需要用户偏好就停下来问。
- 每个建议都必须能解释 evidence、runtime policy 和 preference 来源。
...
Runtime 是核心资产。AI advisor、advanced settings、CLI / JSON / Agent API 和 desktop GUI 都是 runtime 上面的控制层。
...
- 编译和执行 runtime policy。
- 判断 equal / similar / different / unknown / needs_more_analysis。
- 执行 traversal policy。
- 生成 evidence、confidence、risk level。
- 生成 report 和 safe-plan。
- 决定何时 require_confirmation。
- 写入 journal。

AI 接口和高级 settings 都不能绕过 runtime。

### Natural-Language Advisor

面向基础用户，把自然语言偏好翻译成可见、可改、可撤销的规则草案。

### Advanced Settings Editor

面向高级用户，允许直接编辑 runtime policy、predicate、workflow 和 risk threshold。
...
- 高级 settings。
- 自然语言 advisor 生成并确认的规则。
- review decisions。
- workflow 模板。
- 内置高置信规则。

所有规则最终都必须进入 File Organization Runtime。
...
- 引入 `RuntimePolicy`、`Preference`、`ReviewDecision`、`SafePlan`。
- 定义 File Organization Runtime 作为 AI、settings、CLI、agent API 和 GUI 共享的底层语义。
...
现有 `filesage plan <folder>` 是一个非常早期的 CLI spike。早期 `file-organizer` 命令保留为兼容别名，新开发默认使用 `filesage`。
